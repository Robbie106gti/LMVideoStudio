...
import { view, ActivityPanelState_$reflection } from "../ActivityPanel.fs.js";
import { JobEventDto_$reflection, SystemStatusDto_$reflection } from "../Api.fs.js";
...
export function Shell_statusBar(status) {
    let value, elems;
    return createElement("footer", createObj(ofArray([(value = "h-8 border-t border-surface-border bg-surface-raised px-4 flex items-center gap-4 text-xs text-slate-500", ["className", value]), (elems = [createElement("span", {
        children: "LMVideoStudio Phase 0",
    }), defaultArg(map((s) => createElement("span", {
        children: `Host OK · Ollama ${s.Ollama ? "✓" : "—"} · Worker ${s.Worker ? "✓" : "—"}`,
...
export function Shell_chrome(tab, activity, status, content, dispatch) {
    ...
    }), Shell_navButton("Projects", new ShellTab(0, []), tab, dispatch), Shell_navButton("Timeline", new ShellTab(1, []), tab, dispatch), Shell_navButton("Settings", new ShellTab(2, []), tab, dispatch)], ["children", Interop_reactApi.Children.toArray(Array.from(elems))])])))], ["children", Interop_reactApi.Children.toArray(Array.from(elems_1))])]))), createElement("div", createObj(ofArray([["className", "flex flex-1 min-h-0"], (elems_2 = [content, view(activity)], ["children", Interop_reactApi.Children.toArray(Array.from(elems_2))])]))), Shell_statusBar(status)], ["children", Interop_reactApi.Children.toArray(Array.from(elems_3))])])));
}
