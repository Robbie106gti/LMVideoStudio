...
Optional frame generation doubles the selected source rate: 12 to 24 fps for the 4K profile or 24 to 48 fps for the legacy profile. The 4K path pads 1280x704 sources to the sidecar's exact 1280x720 contract before interpolation; the legacy path interpolates before its later 640x352-to-640x360 padding. Timing-aware sidecars receive frame intervals derived from the actual source and output rates instead of prototype constants, with a compatibility retry for qualified legacy sidecars. Both paths use scene-jump detection and hold a real frame instead of interpolating across a cut, but remain opt-in because thin cables and similar geometry can produce doubled edges.
